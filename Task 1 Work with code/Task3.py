...
# Required fields (excluding 'cid' which is optional)
required_fields = {"byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid"}

# Function to parse the input and return a list of passports as dictionaries
def parse_passports(batch_input):
    passports = []
    # Split the input by double newlines to separate individual passports
    raw_passports = batch_input.strip().split("\n\n")
    for raw_passport in raw_passports:
        passport = {}
        # Replace newlines with spaces and then split into key:value pairs
        fields = raw_passport.replace("\n", " ").split()
        for field in fields:
            # Check if field contains a colon and is not empty
            if ':' in field:
                key, value = field.split(":", 1)  # Use maxsplit=1 to avoid multiple colons error
                passport[key] = value
        passports.append(passport)
    return passports

# Function to count the number of valid passports
...
# Parse the passports from the input
passports = parse_passports(batch_input)

# Count and print the number of valid passports
